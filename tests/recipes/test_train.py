...
def train(
    ...
    with RichTablePrinter(
        ...
        # Training loop
        print("Training", ", ".join([name for name, pipe in model.torch_components()]))
        ...
        trained_pipes = torch.nn.ModuleDict(dict(model.torch_components()))
...
def test_function(pdf, error_pdf, change_test_dir, dummy_dataset, tmp_path):
    ...
    model.add_pipe(
        ...
        config={
            ...
            "embedding": {
                "@factory": "embedding_combiner",
                "dropout_p": 0.1,
                "text_encoder": {
                    "@factory": "sub_box_cnn_pooler",
                    "out_channels": 64,
                    "kernel_sizes": (3, 4, 5),
                    "embedding": {
                        "@factory": "simple_text_embedding",
                        "size": 72,
                    },
                },
                "layout_encoder": {
...
def test_script(change_test_dir, dummy_dataset):
    result = runner.invoke(
        ...
        [
            ...
            "4",
            "--output-dir=null",
            "--components.aggregator.@factory=simple_aggregator",
