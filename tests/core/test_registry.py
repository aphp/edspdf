...
def test_factory_default_config():
    @registry.factory.register("custom_test_component_1", default_config={"value": 5})
    ...
    registry_result = registry.factory.get("custom_test_component_1")()
    assert isinstance(registry_result, CurriedFactory)

    pipeline = Pipeline()
    pipeline.add_pipe("custom_test_component_1")

    assert pipeline.get_pipe("custom_test_component_1").value == 5


def test_factory_required_arguments():
    with pytest.raises(ValueError) as exc_info:

        @registry.factory.register("custom_test_component_2")
        class CustomComponent:
