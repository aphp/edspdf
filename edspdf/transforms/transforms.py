...
class ChainTransform(object):
    ...
    def __call__(self, df: pd.DataFrame, *args, **kwargs) -> pd.DataFrame:
        for layer in self.layers:
            df = layer(df, *args, **kwargs)
        return df


def add_telephone(df: pd.DataFrame, *args, **kwargs) -> pd.DataFrame:
...
def add_dates(df: pd.DataFrame, *args, **kwargs) -> pd.DataFrame:
...
def add_dimensions(df: pd.DataFrame, *args, **kwargs) -> pd.DataFrame:
...
def add_orbis(df: pd.DataFrame, orbis: bool, *args, **kwargs) -> pd.DataFrame:
